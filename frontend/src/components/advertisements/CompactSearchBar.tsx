...
const CompactSearchBar = ({
  ...
  return <div className={`${inNav ? 'w-full flex justify-center' : 'top-[64px] mt-[64px] pt-6 pb-2 zoom-out-0 backdrop-blur-sm'}`}>
    <div className={`${inNav ? 'w-full max-w-md' : 'container mx-auto'}`}>
      <form onSubmit={handleSearchSubmit} className="flex items-center justify-between">
        <div className={`w-full ${inNav ? '' : 'max-w-2xl p-4 mx-auto rounded-2xl bg-transparent py-[8px]'}`}>
          <div className={`flex ${inNav ? 'gap-2' : 'flex-col md:flex-row gap-3'}`}>
            ...
            <Button type="submit" className={` bg-green-700 hover:bg-green-800 sm:text-md font-medium ${inNav ? 'h-10 w-10 rounded-full p-0' : 'px-8 h-12 rounded-md'} flex items-center justify-center`}>
