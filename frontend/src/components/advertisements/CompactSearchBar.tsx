...
const CompactSearchBar = ({
  ...
  return (
    <div className={`${inNav ? 'w-full flex justify-center lg:justify-start' : 'top-[64px] mt-[64px] pt-3 sm:pt-6 pb-2 zoom-out-0 backdrop-blur-sm'}`}>
      <div className={`${inNav ? 'w-full max-w-md md:max-w-sm' : 'container mx-auto px-4 sm:max-w-3xl md:max-w-4xl lg:max-w-5xl'}`}>
            ...
            <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 md:flex-row md:gap-2 items-stretch">
              ...
              <div className="flex flex-row gap-2 w-full">
                <div className="relative flex-1">
                  <Search className={`absolute left-3 ${inNav ? 'top-2 h-4 w-4' : 'top-2.5 h-4 w-4 sm:h-5'} text-green-500`} />
                  <Input
                    type="text"
                ...
                <div className="relative flex-1">
                  <MapPin className={`absolute left-3 ${inNav ? 'top-2 h-4 w-4' : 'top-2.5 h-4 w-4 sm:h-5 sm:w-4'} text-green-500`} />
                  <Input
                    type="text"
              ...
              <div className="flex flex-row gap-2 mt-2 sm:mt-0 w-full sm:w-auto">
                <Button
                  type="submit"
                  className={`bg-green-700 hover:bg-green-800 text-[10px] sm:text-xs md:text-sm font-medium ${inNav ? 'h-8 md:h-9 w-8 md:w-9 rounded-full p-0' : 'h-9 sm:h-10 md:h-11 px-3 sm:px-6 md:px-4 rounded-md w-3/4 sm:w-auto md:w-40 lg:w-44'} flex items-center justify-center`}
                >
                  <Search className={`${inNav ? 'h-4 w-4' : 'h-3 w-3 sm:h-4 sm:w-4 mr-1 sm:mr-2'} `} />
                  {inNav ? '' : 'Find Items'}
                </Button>
                {!inNav && (
                  <Button
