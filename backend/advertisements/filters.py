...
class ProductFilter(filters.FilterSet):
    ...
    def filter_availability(self, queryset, name, value):
        ...
        if name == 'start_date':
            return queryset.filter(
                ...
                ~Q(unavailable_dates__range_start__lte=value, unavailable_dates__range_end__gte=value)
            ).distinct()
        elif name == 'end_date':
            return queryset.filter(
    ...
    class Meta:
        model = Product
        fields = ['search', 'location', 'min_price', 'max_price', 'start_date', 'end_date', 'duration_unit', 'ordering']
